...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 进度中心 <span class="c-gray en">&gt;</span> 用户管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" id="reflash" title="刷新" name="reflash" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		 <c:forEach items="${jdList}" var="jd" varStatus="jdStatus">                         
			<tr class="text-c">
				<td><label for="uid${jdStatus.index}">${jdStatus.index+1}</label></td>
				<td><label for="uid${jdStatus.index}">${jd.rwName}</label></td>
				<td><label for="uid${jdStatus.index}">${jd.jindu }</label></td>
				<td><label for="uid${jdStatus.index}">${jd.rwjdStudname}</label></td>
				<td class="td-manage"> <a title="分配" href="javascript:;" onclick="member_edit('分配','${pageContext.request.contextPath}/jindu/updateJindu?id='+${jd.id},${jd.id},'','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">进度管理【&#xe6df;】</i> <a title="删除" href="javascript:;" onclick="member_del(this,${jd.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">删除【&#xe6e2;】</i></a></td>
			</tr>
		 </c:forEach>	
		</tbody>
	</table>
