...
<div class="page-container">
	...
	<div class="text-c"> 
	<form action="${pageContext.request.contextPath}/students/studentList?">日期范围：
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		 <c:forEach items="${newsList}" var="news" varStatus="newsStatus">                         
			<tr class="text-c">
				<td><label for="uid${newsStatus.index}">${newsStatus.index+1}</label></td>
				<td><label for="uid${newsStatus.index}">${news.title }</label></td>
				<td><label for="uid${newsStatus.index}">${news.author}</label></td>
				<td><label for="uid${newsStatus.index}">${news.releaseTime }</label></td>
				<td><label for="uid${newsStatus.index}">${news.content}</label></td>
				<td><label for="uid${newsStatus.index}">${news.keywords}</label></td>
				<td class="td-manage"> <a title="编辑" href="javascript:;" onclick="member_edit('编辑','${pageContext.request.contextPath}/news/updateNews?id='+${news.id},${news.id},'','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">编辑【&#xe6df;】</i> <a title="删除" href="javascript:;" onclick="member_del(this,${news.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">删除【&#xe6e2;】</i></a></td>
			</tr>
		 </c:forEach>	
		</tbody>
	</table>
