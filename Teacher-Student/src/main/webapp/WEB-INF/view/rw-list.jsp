...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		 <c:forEach items="${rwList}" var="rw" varStatus="rwStatus">                         
			<tr class="text-c">
				<td><label for="uid${rwStatus.index}">${newsStatus.index+1}</label></td>
				<td><label for="uid${rwStatus.index}">${rw.rwName }</label></td>
				<td><label for="uid${rwStatus.index}">${rw.rwInfo}</label></td>
				<td><label for="uid${rwStatus.index}">${rw.comeDate}</label></td>
				<td><label for="uid${rwStatus.index}">${rw.studentName }</label></td>
				<td class="td-manage"> <a title="分配" href="javascript:;" onclick="member_edit('分配','${pageContext.request.contextPath}/renwu/updateRenwu?id='+${rw.id},${rw.id},'','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">分配【&#xe6df;】</i> <a title="删除" href="javascript:;" onclick="member_del(this,${rw.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">删除【&#xe6e2;】</i></a></td>
			</tr>
		 </c:forEach>	
		</tbody>
	</table>
