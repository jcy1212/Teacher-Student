...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 新闻中心 <span class="c-gray en">&gt;</span> 用户管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" id="reflash" title="刷新" name="reflash" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">

	<div class="text-c"> 
	<form action="${pageContext.request.contextPath}/renwu/rwList?">日期范围：
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="member_add('添加用户','${pageContext.request.contextPath}/students/addStudent','500','300')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加维修</a></span> <span class="r">共有数据：<strong>${num }</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		 <c:forEach items="${bxList}" var="bx" varStatus="bxStatus">                         
			<tr class="text-c">
				<td><label for="uid${bxStatus.index}">${bxStatus.index+1}</label></td>
				<td><label for="uid${bxStatus.index}">${bx.wxName }</label></td>
				<td><label for="uid${bxStatus.index}">${bx.wxInfo}</label></td>
				<td><label for="uid${bxStatus.index}">${bx.comeDate}</label></td>
				<td class="td-manage"> <a title="分配" href="javascript:;" onclick="member_edit('分配','${pageContext.request.contextPath}/renwu/updateRenwu?id='+${bx.id},${bx.id},'','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">分配【&#xe6df;】</i> <a title="删除" href="javascript:;" onclick="member_del(this,${bx.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">删除【&#xe6e2;】</i></a></td>
			</tr>
		 </c:forEach>	
		</tbody>
	</table>
