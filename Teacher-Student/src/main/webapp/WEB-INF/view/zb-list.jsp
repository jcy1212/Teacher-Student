...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 值班管理 <span class="c-gray en">&gt;</span> 值班管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" id="reflash" title="刷新" name="reflash" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		 <c:forEach items="${zbList}" var="zb" varStatus="zbStatus">                         
			<tr class="text-c">
				<td><label for="uid${zbStatus.index}">${newsStatus.index+1}</label></td>
				<td><label for="uid${zbStatus.index}">${zb.zbName }</label></td>
				<td><label for="uid${zbStatus.index}">${zb.zbPlace}</label></td>
				<td><label for="uid${zbStatus.index}">${zb.zbDate }</label></td>
				<td class="td-manage"> <a title="编辑" href="javascript:;" onclick="member_edit('编辑','${pageContext.request.contextPath}/zb/updateZb?id='+${zb.id},${zb.id},'','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">编辑【&#xe6df;】</i> <a title="删除" href="javascript:;" onclick="member_del(this,${zb.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">删除【&#xe6e2;】</i></a></td>
			</tr>
		 </c:forEach>	
		</tbody>
	</table>
